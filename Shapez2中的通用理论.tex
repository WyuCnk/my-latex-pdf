...
\begin{document}
	...
	\section{基础定义}
		...
		\subsection{颜色无关性}
			...
			\begin{definition}[块的空间结构]
				定义块的\textbf{空间结构}为其类型分量。
				即对块
				$$
				...
				定义
				$$
				\operatorname{sh}(b)=\tau.
				$$
			\end{definition}
			
			\begin{definition}[图形的空间结构]
				设$S\in\mathbb{S}$。
				定义$S$的\textbf{空间结构}为
				$$
				\operatorname{Sh}(S)
				...
				(\tau_p)_{p\in I_S}.
				$$
				也就是说，$\operatorname{Sh}(S)$仅记录每个位置处块的类型，而忘记其颜色。
