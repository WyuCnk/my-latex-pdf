	...
	\section{基础机制}
		...
		\subsection{下落过程的不动点刻画}
			...
			\begin{proof}
				...
				首先，任意触地组都不可能下落，因此
				$$
				\mathcal{B}_S\subseteq\mathcal{X}.
				$$
				若$G\in\mathcal{X}$且$G\to_S H$，则在初始状态中$H$受到$G$的支撑。
				由于$G$始终不发生下落，并且全局下落过程不产生外部重粘连，$G$对$H$的阻挡不会因其他下落组的运动而消失。
				因此$H$也不可能先于$G$发生下落，从而$H\in\mathcal{X}$。
				故
				$$
				\Theta_S(\mathcal{X})\subseteq\mathcal{X}.
				$$
				又由$\mathcal{B}_S\subseteq\mathcal{X}$以及上述支撑闭包性，实际上有
				$$
				\Theta_S(\mathcal{X})=\mathcal{X}.
				$$
				因此$\mathcal{X}$是$\Theta_S$的不动点。
				于是
				$$
				\mathcal{X}\subseteq\nu\Theta_S.
				$$
				...
				由最大不动点的循环刻画，$\nu\Theta_S$由触地组、循环支撑核以及它们沿支撑关系向上可达的组组成。
				
				触地组不能下落；
				循环支撑结构中的组相互支撑，因而没有任何一个组能够首先下落；
				而由这些组向上支撑到的组始终受到下方不下落组的阻挡。
				根据下落过程的不重粘连原则，其他下落组的运动不会破坏这些初始支撑依赖。
				因此$\nu\Theta_S$中的初始最大粘连组均不会发生第一次下落。
				于是
				$$
	...
	\section{空间结构与可制作性}
		...
		\subsection{可制作图形与制作路径}
			...
			\begin{corollary}[颜色参数不影响空间可制作性]\label{cor:make-shape-color-independence}
				设存在双射
				$$
				\rho:\mathcal{I}_0\to\mathcal{I}'_0
				$$
				其中$\mathcal{I}_0\subseteq\mathcal{I}$、$\mathcal{I}'_0\subseteq\mathcal{I}'$，并且对任意$S\in\mathcal{I}_0$有
				$$
